...
> **v1.0 已在真机完整验证**（一加 PLQ110 / Android 16 / KernelSU / LSPosed v2.1.1）：开屏 / 健康 / 运动 / 设备 / 我的 五处广告全部清除，正常功能完好。v1.0 起启用 R8 压缩 + 移除 appcompat 死重，APK 体积 3.26 MB → **29 KB**。
...
```powershell
gradle assembleRelease   # R8 + shrinkResources 已启用，产物 app/build/outputs/apk/release/app-release.apk
```

Release 使用 debug 签名（LSPosed 模块无需正式签名）。R8 混淆后入口类名随 `java_init.list` 自适应（`-adaptresourcefilecontents`，libxposed 官方机制）。
